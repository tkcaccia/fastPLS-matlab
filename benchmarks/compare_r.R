...
prediction_at <- function(object, index = 1L) {
    value <- object$Ypred
    if (is.list(value)) return(value[[index]])
...
for (method in c("simpls", "plssvd", "opls", "kernelpls")) {
    ...
    for (iteration in seq_len(repetitions)) {
        ...
        fit <- pls(Xtrain, Ytrain, Xtest, ncomp = 8, method = method,
            backend = "cpu", scaling = "centering", return_variance = FALSE,
            oversample = 32L, power = 5L, seed = 17L,
            north = 1L, kernel = "rbf", gamma = 0.1)
        elapsed[[iteration]] <- proc.time()[["elapsed"]] - started
    }
    write.csv(as.matrix(prediction_at(fit)), file.path(output_dir,
        paste0("r_", method, "_regression.csv")), row.names = FALSE)
    writeLines(format(median(elapsed), digits = 17), file.path(output_dir,
        paste0("r_", method, "_seconds.txt")))
    classification <- pls(Xtrain, labels, Xtest, ncomp = 8, method = method,
        classifier = "lda", backend = "cpu", scaling = "centering",
        return_variance = FALSE, oversample = 32L,
        power = 5L, seed = 17L, north = 1L,
        kernel = "rbf", gamma = 0.1)
    writeLines(as.character(prediction_at(classification)), file.path(output_dir,
        paste0("r_", method, "_labels.txt")))
}
